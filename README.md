...
1. `npm install`
2. Запустить `./supabase/supabase.exe start` (поднимает Postgres + Auth + Storage в Docker)
3. Опционально `./supabase/supabase.exe db reset`
4. После старта CLI напечатает `API URL` и `anon key` — положи их в `.env.local`:
   ```
   VITE_SUPABASE_URL=http://127.0.0.1:54321
   VITE_SUPABASE_ANON_KEY=<твой anon key>
   ```
5. `npm run dev` → откроется `http://localhost:5173`
6. Тестовые пользователи — пароль `Test123456`, email в `supabase/seed.sql`
...
1. Проверяют/запускают Docker (пробуют установить через Homebrew, если нет).
...
```bash
# macOS: запустить Docker Desktop.app (или `brew install --cask docker` если нет)
