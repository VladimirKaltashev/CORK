    ...
    <div className="mx-auto max-w-2xl py-6 px-3">
      ...
      <div className="flex border-b border-gray-200 mb-4">
        ...
        >
          Мои друзья
          {friends.length > 0 && (
            <span className="ml-1.5 text-xs text-gray-400">({friends.length})</span>
          )}
        </button>
        <button
          type="button"
      ...
      {isLoading ? (
        ...
        friends.length === 0 ? (
          <div className="border border-dashed border-gray-300 rounded-md py-8 text-center">
            <span className="text-sm text-gray-500">Друзей пока нет. Найдите людей через поиск на ленте!</span>
          </div>
        ) : (
